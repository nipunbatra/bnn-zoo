...
def plot_prediction(X,Y,x_stack,y_stack,mean,sigma,title,ax=None,n_points=300):
    ...
    for i_std in range(1,4):
      ax.fill_between(x_stack.reshape(n_points), jnp.array((mean-i_std*sigma)), jnp.array((mean+i_std*sigma)), color='orange',alpha=2/(3*i_std), label=f'$\mu\pm{i_std}\sigma$')
    ax.scatter(x_stack[:int(n_points/3)], y_stack[:int(n_points/3)],color='blue',alpha=0.5)
    ax.scatter(X, Y,color='black',alpha=0.7)
    ax.scatter(x_stack[int(n_points*2/3):], y_stack[int(n_points*2/3):],color='blue',alpha=0.5)
