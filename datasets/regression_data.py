...
import numpy as np
def data_set(n_points=100, xrange=3, std=3.,poly=False,hetero=False):
    xx =jnp.linspace(-1*xrange,1*xrange ,n_points).reshape(-1, 1)
    x_test_1 = jnp.linspace(-1*(xrange+1),-1*xrange,int(n_points)).reshape(-1, 1) 
    x_test_2 = jnp.linspace(xrange,xrange+1,int(n_points)).reshape(-1, 1) 
    if(poly):
